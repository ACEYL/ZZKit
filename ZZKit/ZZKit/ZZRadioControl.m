...
-(void)defaultInit{
    ...
    [_button setImage:RADIO_UNSELECTEDIMAGE forState:UIControlStateNormal];
    [_button setImage:RADIO_SELECTEDIMAGE forState:UIControlStateSelected];
    //添加按钮监听事件
    ...
    [ZZRadioControl registerInstance:self];
}
#pragma mark - Manage Instances
//添加选中的RadioButton
+(void)registerInstance:(ZZRadioControl *)radioButton{
    if(!rb_instances){
...
-(void)handleButtonTap:(id)sender{
    ...
    [ZZRadioControl buttonSelected:self];
}

#pragma mark - Class level handler
//RadioButton选中后调用的事件
+(void)buttonSelected:(ZZRadioControl*)radioButton{
    // 通知观察者
    if (rb_observers) {
        //获得观察者对象
    ...
    if (rb_instances) {
        for (int i = 0; i < [rb_instances count]; i++) {
            ZZRadioControl *button = [rb_instances objectAtIndex:i];
